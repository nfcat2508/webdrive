defmodule WebdriveWeb.UploadChannel do
  use Phoenix.Channel
    ...
    with {:ok, %{pid: pid, ref: ref, cid: cid}} <- Static.verify_token(socket.endpoint, token),
      ...
      socket =
        assign(socket, %{
          ...
          live_view_pid: pid,
          max_file_size: max_file_size * 1.01 + 100,
  ...
  def handle_in("chunk", {:binary, payload}, socket) do
    ...
    if !socket.assigns.writer_closed? and byte_size(payload) + uploaded_size <= max_file_size do
      ...
      Logger.warning(
        "max_file_size: #{max_file_size}, current_size: #{byte_size(payload) + uploaded_size}"
      )
  ...
  @impl true
  def handle_in("finished", _, socket) do
    case close_writer(socket, :done) do
      {:ok, socket} ->
        {:reply, :ok, assign(socket, done?: true)}

      {:error, reason, new_socket} ->
        {:reply, {:error, %{reason: reason}}, new_socket}
    end
  end
